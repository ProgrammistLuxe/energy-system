...
import { GenerateDiffService } from '@pages/schemas/services/generate-diff.service';
...
import { Difference } from '@pages/schemas/models/difference.model';
...
export class ElementDataDialogComponent {
  ...
  async saveDiff() {
    ...
    res
      ...
      .subscribe((response) => {
        ...
        if (!this.schemaService.selectedSchema || !response.result.data.length) {
          return;
        }
        const computedDiff: DiffItem = {
          diff_id: response.result.data[0].id,
          diff_name: response.result.data[0].object_name,
          schema_uid: this.schemaService.selectedSchema.uid,
          place_name: this.schemaService.selectedSchema.name,
        };
  ...
  ngOnInit() {
    const id: string = this.data['attributes']['attrs']?.['rdf']?.['diagramObjectId'];
    if (!id) {
      return;
    }
