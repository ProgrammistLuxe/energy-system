...
export interface AttrFormGroup {
...
@Component({
  ...
  styleUrl: './passport-template.component.scss',
})
export class PassportTemplateComponent {
  ...
  getNode() {
    ...
    this.passportTemplateService
      ...
      .subscribe((response) => {
        ...
        this.setControlsValues(attrSet);
      });
  }
  saveNode() {
    ...
    this.passportTemplateService
      .updateTreeTemplate(this.nodeId, reqData)
      .pipe(
        takeUntil(this.destroy$),
  ...
  private setControlsValues(attrSet: number[]) {
    if (!this.currentNode) {
      return;
    }
    this.attributes.clear();
    this.templateName.setValue(this.currentNode.title);
